...
static __constant int morton256_x[256] =
{
    0x00000000,
...
static __constant int morton256_y[256] = {
    0x00000000,
...
static __constant int morton256_z[256] = {
    0x00000000,
...
void vcurve2coords(vcurve_t vcurve, vside_t side, vside_t* x, vside_t* y, vside_t* z)
{
    //UNUSED(side);
...
vcurve_t coords2vcurve(vside_t x, vside_t y, vside_t z, vside_t side)
{
    //UNUSED(side);

    int index = morton256_x[x] | morton256_y[y] | morton256_z[z];
    return index;
}


/*ccurve_t corner2ccurve(corner_t corner)
{
    return get_corner_index(corner);
}

corner_t ccurve2corner(ccurve_t ccurve)
{
    assert(ccurve < 8);
    return get_corner_by_index(ccurve);
}*/

vcurvesize_t vcurvesize(vside_t side)
{
    const vcurvesize_t vcurvesizemax = -1;
    const vcurvesize_t vcurvesidemax = vcurvesizemax/3;


    vcurvesize_t side_ = side;
...
genZeroCross(__global cl_edge_t* edge_array, cl_float3_t base_pos, float res, uchar level)
{
	...
	const float3 p = findCrossingPoint(1, e1, e2, level);
...
test(__global int* densities, cl_float3_t world_pos, float res, uchar level)
{
	...
	vcurve_t vcurve = coords2vcurve(coord.x, coord.y, coord.z, size);

	const float3 position = (float3)((world_pos.x + coord.x * res),
                                     (world_pos.y + coord.y * res),
                                     (world_pos.z + coord.z * res));

    densities[vcurve] = densities3d(position, level) < 0.0f ? 1 : 0;
