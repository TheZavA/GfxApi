...
#define checkOpenGLError() checkOglError(__FILE__, __LINE__)
	...
	GLenum toGL(TextureType texturetype)
	{
		...
		case (TextureType::Texture3D) : return GL_TEXTURE_3D;
			...
			//case (TextureType::TextureCubeMap): return ;
			//case (TextureType::TextureCubeMapArray): return ;
	...
	VertexDataType VertexElement::Type() const
	{
		return m_type;
	}
	...
	IndexBuffer::IndexBuffer(boost::weak_ptr<Mesh> mesh, int numIndices, PrimitiveIndexType type, Usage usage, bool allocateCpu)
		: m_mesh(mesh)
	...
	void Mesh::GenerateVAO(int startVertexOffset)
	{
		...
		///Fill in declaration.elements
		{
			BOOST_FOREACH(const VertexBuffer& vb, vbs | indirected)
			{
				BOOST_FOREACH(const VertexElement& element, vb.Declaration().Elements())
		...
		int attr_index = 0;
		///vbo
		BOOST_FOREACH(VertexBuffer& vb, vbs | indirected)
		{
			/**
			...
			*/
			...
			BOOST_FOREACH(const VertexElement& element, dec.Elements())
			{
				...
				if (element.Type() == VertexDataType::FLOAT)
				{
...
#define FOURCC_DXT5 0x35545844 // Equivalent to "DXT5" in ASCII
	...
	{
		static boost::shared_ptr<ShaderProgram> sp;
		if (!sp)
		{
			boost::shared_ptr<Shader> vs = boost::make_shared<Shader>(ShaderType::VertexShader);
			...
			boost::shared_ptr<Shader> ps = boost::make_shared<Shader>(ShaderType::PixelShader);
			...
			sp = boost::make_shared<ShaderProgram>();
			sp->Attach(*vs);
			sp->Attach(*ps);
