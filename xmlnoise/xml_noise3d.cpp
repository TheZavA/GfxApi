#include "xml_noise3d.hpp"

#include "xml_noise_decls.hpp"


xml_noise3d_t::xml_noise3d_t(noisepp::Pipeline3D& pipeline)
    : pipeline(pipeline)
    , root(NULL)
{
...
xml_noise3d_t::~xml_noise3d_t()
{
}

//Simple recursive version, for testing. Broken ATM.
void xml_noise3d_t::simple_load(const std::string& xml_file_name)
{
...
module_ptr_t xml_noise3d_t::simple_traverse(const tinyxml2::XMLElement& element)
{
    ...
    //Visit current
    {
        ...
        handlers3d_t::const_iterator w = handlers.find(name);
        ...
        const tag_handler3d_t& handler = w->second;
...
void xml_noise3d_t::load(const std::string& xml_file_name)
{
    using namespace tinyxml2;
...
void xml_noise3d_t::visit(const tinyxml2::XMLElement& element)
{
    using namespace tinyxml2;
    using namespace noisepp;
