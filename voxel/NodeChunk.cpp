...
void NodeChunk::classifyChunk()
{
   if( m_pChunkManager->m_ocl->classifyChunk( m_chunk_bounds.minPoint.x,
                                              m_chunk_bounds.minPoint.y,
                                              m_chunk_bounds.minPoint.z,
                                              m_scale ) )
      m_bHasNodes = true;
...
void NodeChunk::generateLocalAO()
{
   ...
   //int tree_level = m_pTree->getLevel();
   //m_pChunkManager->m_ocl->calulateGlobalAO( m_vertices.size(), m_vertices,
   //                                          m_chunk_bounds.minPoint.x,
   //                                          m_chunk_bounds.minPoint.y,
   //                                          m_chunk_bounds.minPoint.z,
   //                                          m_scale,
...
void NodeChunk::createMesh()
{
   ...
   decl.Add( GfxApi::VertexElement( GfxApi::VertexDataSemantic::TCOORD, GfxApi::VertexDataType::UNSIGNED_BYTE, 1, "vertex_gao" ) );
   ...
   for( auto& vertexInf : m_vertices )
   {
      ...
      vbPtr[offset + 4] = vertexInf.globalAO;
