...
/*
 * This code is derived from the reference implementation of DC which can be found here:
 * http://sourceforge.net/projects/dualcontouring/
 * This code is free for any non-commercial use.
 */

#define ROTATE(a,i,j,k,l) g=a[i][j];h=a[k][l];a[i][j]=g-s*(h+g*tau);a[k][l]=h+s*(g-h*tau);

void jacobi ( const float3x3& u, float3& d, float3x3& v)
{
	int j, iq, ip, i;
...
void matInverse ( float3x3& u, float3& w, float rvalue[][3])
{
	if ( w [ 0 ] == 0.0f )
	{
...
float calcError ( const float3x3 a, const float b[], float btb, const float point[] )
{
	float rvalue = btb;
